...
namespace DXEngine {
    ...
    void PickingManager::RegisterPickable(std::shared_ptr<IPickable> pickable)
    {
        if (pickable)
        {
    ...
    void PickingManager::UnRegisterPickable(std::shared_ptr<IPickable> pickable)
    {
        m_Pickables.erase(
            std::remove_if(m_Pickables.begin(), m_Pickables.end(),
                [&pickable](const std::weak_ptr<IPickable>& weak) {
    ...
    HitInfo PickingManager::Pick(float screenX, float screenY, int screenWidth, int screenHeight, const Camera& camera)
    {
        ...
        std::shared_ptr<IPickable> hitObject = nullptr;
    ...
    void PickingManager::SetPickedObject(std::shared_ptr<IPickable> pickable)
    {
    ...
    void PickingManager::CleanupExpiredPointers()
    {
        m_Pickables.erase(
            std::remove_if(m_Pickables.begin(), m_Pickables.end(),
                [](const std::weak_ptr<IPickable>& weak) {
                    return weak.expired();
                }),
            m_Pickables.end()
        );
