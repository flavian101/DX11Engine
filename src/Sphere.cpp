...
//   }
    ...
    for (size_t i = 0; i < indices.size(); i += 3)
    {
        ...
        // Position deltas
        XMVECTOR edge1 = XMVectorSet(p1.x - p0.x, p1.y - p0.y, p1.z - p0.z, 0);
        XMVECTOR edge2 = XMVectorSet(p2.x - p0.x, p2.y - p0.y, p2.z - p0.z, 0);
        ...
        XMVECTOR sdir = (edge1 * dV2 - edge2 * dV1) * r;
        XMVECTOR tdir = (edge2 * dU1 - edge1 * dU2) * r;

        XMFLOAT3 sd, td;
        XMStoreFloat3(&sd, sdir);
        XMStoreFloat3(&td, tdir);

        // Accumulate for each of the triangle’s vertices
        auto accumulate = [&](unsigned idx, const XMFLOAT3& f) {
    ...
    for (size_t i = 0; i < vCount; ++i)
    {
        ...
        XMVECTOR T = XMVector3Normalize(t - n * XMVector3Dot(n, t));
